...
public class SettingCmd extends BaseStatCmd {
    ...
    public Object cmds(XLHttpRequest request, XLHttpResponse response) throws Exception {
        ...
        Collection<Entry<CmdMeta, List<String>>> entry = cmd_urls_map.entrySet();
        Collection<Entry<CmdMeta, List<String>>> lastEntry = last10min_urls_map != null ? last10min_urls_map.entrySet() : null;
        // 按照各个字段排序
        // 按请求比例排序
        if (timesOrder) {
    ...
    public Object setConfig(XLHttpRequest request, XLHttpResponse response) throws Exception {
        ...
        for (Entry<String, List<String>> e : request.getParameters().entrySet()) {
